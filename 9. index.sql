-- pk, fk, unique 제약 조건 추가 시에 해당 컬럼에 대해 index 페이지 자동생성.
...
-- index를 통해 조회성능향상을 얻으려면 반드시 where 조건에 해당 컬럼에 대한 조건이 있어야함.
select * from author where email="hongildong@naver.com";

-- 만약 where조건에서 2컬럼으로 조회시에 1컬럼에만 index가 있다면.
select * from author where name='hong' and email="hongildong@naver.com";


-- 만약 where 조건에서 2컬럼으로 조회시에, 2컬럼 각각의 index가 있다면.
-- 이 경우 db엔진에서 최적의 알고리즘 실행.
select * from author where name='hong' and email="hongildong@naver.com";
-- index는 한 컬럼 뿐만 아니라, 2컬럼을 대상으로 1개의 index를 설정하는 것도 가능.
-- 이 경우 두 컬럼을 and조건으로 조회해야만 index를 사용한다.
-- 복합 인덱스 생성
create index 인덱스명 on 테이블명(컬럼1, 컬럼2);

-- 기존 테이블 삭제 후 아래 테이블로 신규 생성
create table author(id bigint auto_increament primary key, email varchar(255), name varchar(255));

-- index 테스트 시나리오
-- 아래 프로시저를 통해 수십만건의 데이터 insert 후에 index 생성 전후에 따라 조회 성능확인
DELIMITER //
...
BEGIN
    ...
    WHILE iteration <= max_iterations DO
        ...
        WHILE i <= iteration * batch_size DO
            SET email = CONCAT('sejin', i, '@naver.com');
