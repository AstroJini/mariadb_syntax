...
alter table post add constraint post_fk primary key(author_id) references author(id);

-- 기존 제약조건 삭제
-- 새로운 제약조건 추가 

-- on delete/update 제약조건 테스트
-- 부모 테이블 데이터 delete시에 자식 fk컬럼 set null, update시에 자식 fk컬럼 cascade
...
alter table author modify column name varchar(255) default 'annoymous';
-- auto_increment : 입력을 안했을 때 자동으로 증가된 숫자값을 적용
-- 입력을 안했을 때 마지막에 입력된 가장 큰값에서 +1만큼
